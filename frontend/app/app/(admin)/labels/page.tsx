...
function CustomerSearch({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const suggestions = useMemo(() =>
    input.trim() === ''
      ? ALL_CUSTOMERS
      : ALL_CUSTOMERS.filter(c => c.toLowerCase().includes(input.toLowerCase())),
    [input]
  )
  ...
  return (
    <div className={styles.acWrap} ref={wrapRef}>
      <div className={styles.acInputWrap}>
        ...
        />
        {input && (
          <button className={styles.acClear} onClick={clear} tabIndex={-1}>✕</button>
        )}
      </div>
      {open && suggestions.length > 0 && (
        <ul className={styles.acList}>
          <li className={styles.acAll} onMouseDown={() => { clear(); }}>
            All customers
          </li>
          {suggestions.map(c => (
            <li
              key={c}
              className={`${styles.acItem} ${input === c ? styles.acSelected : ''}`}
...
// bold-highlight matched portion
...
export default function LabelsPage() {
  ...
  const filtered = useMemo(() => MOCK.filter(s => {
    ...
    return true
  }), [tracking, customer, service, codOnly, dateFrom, dateTo])
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.filters}>
        <input
          className={styles.input}
          placeholder="Search tracking number (partial ok)…"
          value={tracking}
