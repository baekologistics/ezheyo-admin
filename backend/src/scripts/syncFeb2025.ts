/**
 * syncFeb2025.ts
 * Sync February 2025 orders day by day (sdate == edate).
 */
...
async function main() {
  const dates: string[] = []
  for (let d = 1; d <= 28; d++) {
    dates.push(`2025-02-${String(d).padStart(2, '0')}`)
  }

  console.log(`Syncing ${dates.length} days (2025-02-01 ~ 2025-02-28)\n`)
  ...
  const total = await pool.query(
    `SELECT COUNT(*) FROM orders WHERE date >= '2025-02-01' AND date <= '2025-02-28'`
  )
  console.log(`\ntotal_feb2025: ${total.rows[0].count}`)
}
