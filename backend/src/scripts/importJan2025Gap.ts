/**
 * importJan2025Gap.ts
 * Import missing 2025-01-28 orders from XLS (regex parser).
 */
...
const FILE         = '/Users/js/Downloads/종합현황(20250101_20250131).xls'
const TARGET_DATES = new Set(['2025-01-28'])
...
async function main() {
  ...
  const res = await pool.query(`
    ...
    WHERE date>='2025-01-01' AND date<='2025-01-31' GROUP BY date ORDER BY date`)
  console.log('\n=== January 2025 final ===')
  const EXCEL: Record<string,number> = {
    '2025-01-02':43,'2025-01-03':43,'2025-01-04':4,'2025-01-06':31,'2025-01-07':33,
    '2025-01-08':66,'2025-01-09':64,'2025-01-10':48,'2025-01-11':1,'2025-01-13':45,
    '2025-01-14':50,'2025-01-15':64,'2025-01-16':39,'2025-01-17':60,'2025-01-18':66,
    '2025-01-20':40,'2025-01-21':78,'2025-01-22':38,'2025-01-23':40,'2025-01-24':57,
    '2025-01-26':2,'2025-01-27':50,'2025-01-28':51,'2025-01-29':47,'2025-01-30':46,'2025-01-31':50,
  }
  for(const row of res.rows as Array<{date:string;cnt:string}>) {
    const d=row.date.slice(0,10); const t=EXCEL[d]??0; const diff=parseInt(row.cnt)-t
    console.log(`  ${d}: ${row.cnt} / ${t} ${diff===0?'✓':diff>0?'+'+diff+' (excess)':diff+' (short)'}`)
  }
  const total = await pool.query("SELECT COUNT(*) FROM orders WHERE date>='2025-01-01' AND date<='2025-01-31'")
  console.log(`\nJanuary 2025 total: ${total.rows[0].count} / 1156`)
}
