/**
 * importMayGap.ts
 * Import missing May 2024 orders using regex-based parser (handles missing </tr> tags).
 */
...
const FILE = '/Users/js/Downloads/종합현황(20240501_20240531).xls'
...
/** Extract all 1Z... tracking numbers from anchor text */
...
/** Extract numeric-only ref number (outside <a> tags) */
...
function parseFile(filePath: string): ParsedOrder[] {
  ...
  // Split on <tr> boundaries (file has no </tr> closing tags)
...
async function main() {
  ...
  const allOrders = parseFile(FILE)
  console.log('Total parsed:', allOrders.length)

  // Distribution by date
  const byDate: Record<string, number> = {}
  for (const o of allOrders) byDate[o.date] = (byDate[o.date] ?? 0) + 1
  console.log('Date distribution:')
  Object.entries(byDate).sort().forEach(([d, n]) => console.log(`  ${d}: ${n}`))

  // Existing trackings in DB
  ...
  const res = await pool.query(`
    ...
    WHERE date >= '2024-05-01' AND date <= '2024-05-31' GROUP BY date ORDER BY date`)
  console.log('\nMay 2024 DB by date:')
  for (const r of res.rows as Array<{ date: string; cnt: string }>) {
    process.stdout.write(`  ${r.date.slice(0,10)}: ${r.cnt}\n`)
  }

  const total = await pool.query("SELECT COUNT(*) FROM orders WHERE date >= '2024-05-01' AND date <= '2024-05-31'")
  console.log(`\nMay 2024 total: ${total.rows[0].count} / 945`)
}
